...
round(sum(is.na(noaa_data$prcp)) / nrow(noaa_data) * 100, 1)
```

    ## [1] 5.6

``` r
sum(is.na(noaa_data$tmin)) / nrow(noaa_data)
```

    ## [1] 0.4371264

``` r
sum(is.na(noaa_data$tmax)) / nrow(noaa_data)
```

    ## [1] 0.4371025

``` r
sum(is.na(noaa_data$snow))/ nrow(noaa_data)
```
