...
brfss_data %>% 
  ...
  ggplot(aes(x = year, y = n_locations, color = state)) + 
    geom_path() + 
    labs(
      title = "Number of Resturant Health Inspections per State (2002 - 2010)",
      x = "Year",
...
instacart_items_per_aisle = 
  ...
  summarize(n_items = n()) %>%
  arrange(desc(n_items)) %>% 
  rename(Aisle = aisle, "Number of Items" = n_items) %>% 
  head(10)

table_for_plot = tableGrob(instacart_items_per_aisle)

grid.arrange(plot_items_per_aisle, table_for_plot, nrow=1, 
